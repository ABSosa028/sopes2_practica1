...
SYSCALL_DEFINE0(fetch_time_syscall)
{
	struct timespec64 times64;
	ktime_get_real_ts64(&times64);
	return times64.tv_sec;
}
SYSCALL_DEFINE0(fetch_time_seconds)
{
	struct timespec64 times64;
	ktime_get_boottime_ts64(&times64);
	return times64.tv_sec;
}
...
SYSCALL_DEFINE2(get_ultimos_messages, char __user *, buffer, size_t, size){
	...
	while (kmsg_dump_get_line(&iter, true, log_buf + largo, size - largo, &log)){
		largo += log;
		if(largo >= size || log == 0)
			break;
	}
	if(copy_to_user(buffer, log_buf, largo)){
		kfree(log_buf);
		return -EFAULT;
	}
	kfree(log_buf);
